...
export default function Home() {
  ...
  const [imageURL2, setImageURL2] = useState<any>('https://media.discordapp.net/attachments/1298032353234653234/1298038016215879700/eric.viinny_apenas_recrie_a_imagem_em_formato_de_cartoon_4k_ult_a853bb8e-9ccc-4d02-ae90-9daf801fa57a.png?ex=65193544&is=6517e3c4&hm=988d2f3dc418f6bb93317403376a7bb4f5bd9073170108cfe0203f4a1e4efa8a&=&width=662&height=662');
  ...
  const [numberField, setNumberField] = React.useState(0);
...
async function sendPrompt() {
  ...
  console.log(selectedTheme.images);

  // Extrair o número da string no formato "/Scala/10.png"
  const imagePath = selectedTheme.images; // Exemplo: "/Scala/10.png"
  const extractedValue = imagePath.split('/')[2].split('.')[0]; // Resultado: "10"

  // Salvar no cache do navegador (localStorage)
  localStorage.setItem('selectedImage', extractedValue);

  // Teste o valor armazenado
  console.log("Valor extraído e salvo no cache:", extractedValue);

  on2Open();

  socketio.emit('generate_image', {
    tema: { ...selectedTheme },
    user: { genero: 'masculino', image: imageURL },
  ...
  async function handleGallerySelectImageByWebCam(imagedata: GalleryType) {
    setImageURL(`${imagedata.image}`);
    setImageURL2(`${imagedata.image}`);
    setUserData({ ...userData, image: imagedata.nome });
    onGalleryClose();
    handleClosePrintingModal();
  }
  ...
  async function printNumberToConsole(number: number): Promise<void> {
    if (!socketio) return;
    socketio.emit('print', number);
    console.log(`Valor do campo: ${number}`);
  }
  ...
  async function TakePhoto() {
    ...
    const interval = setInterval(() => {
      setCountdown((prev) => {
        if (prev === 1) {
          clearInterval(interval); // Para o intervalo ao chegar em 0
          setCountdown(null); // Reseta a contagem
          socketio.emit('take_photo'); // Emite o evento para o servidor
        }
        return prev! - 1; // Decrementa a contagem
      });
  ...
  useEffect(() => {
    ...
    socket.on('get_photo_webcam', (data) => {
      handleGallerySelectImageByWebCam(data)
    });
  ...
  function resetNumberField() {
    setNumberField(0); // Zera o valor do estado
    console.log("Campo de número foi reiniciado para 0.");
  }
  ...
  // Função para desativar a webcam
  const stopWebcam = () => {
  ...
  // Controla a ativação e desativação da câmera com base no estado do modal
  useEffect(() => {
    if(controlCam){  
      if (isOpen) {
        startWebcam();
    ...
    // Cleanup para desativar a câmera caso o componente seja desmontado
    return () => stopWebcam();
  }, [isOpen, controlCam]);
        ...
        <div className="sm:flex gap-4 mb-4 items-center">
          ...
          >
            ...
            <CardFooter className="justify-between before:bg-white/10 border-white/20 border-1 overflow-hidden py-1 absolute before:rounded-xl rounded-large bottom-1 w-[calc(100%_-_8px)] shadow-small ml-1 z-10">
              ...
              <Button onPress={reset} className="text-tiny text-white bg-black/20" variant="flat" color="default" radius="lg" size="sm">
                Tirar nova foto
              </Button>
            </CardFooter>
          </Card>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2 ml-[10px] mt-2">
              ...
              <Chip color="primary" variant="flat">#Scala</Chip>
            </div>
  ...
  size="5xl"
  style={{
    height: "70vh", // Define a altura do modal
    maxHeight: "90vh", // Impede que ultrapasse o viewport
  }}
>
  <ModalContent>
    {(onClose) => (
      <>
        <ModalHeader className="flex flex-col gap-1 items-center">
          <h1 className="text-lg font-bold text-center">IA • Adicionar Usuário</h1>
        </ModalHeader>
          ...
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <video
              ...
              style={{
                ...
                height: "auto", // Mantém a proporção sem cortes
                maxHeight: "70%", // Garante que o vídeo não ultrapasse 70% da altura do modal
                objectFit: "contain", // Evita cortes e mantém todo o vídeo visível
              }}
            ></video>

            {/* Contagem regressiva sobreposta */}
            {countdown !== null && (
              <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white text-4xl font-bold bg-black bg-opacity-50 px-4 py-2 rounded-lg">
                {countdown}
              </div>
            )}
          </div>
        ...
        <ModalFooter>
          ...
          <Button
            color="primary"
            onPress={TakePhoto}
            isDisabled={countdown !== null} // Desabilita o botão enquanto está na contagem
          >
            Confirmar
          </Button>
        </ModalFooter>
...
<Modal isOpen={is2Open} onOpenChange={on2OpenChange} isDismissable={false} hideCloseButton={true}>
  <ModalContent>
    {(on2Close) => (
        ...
        <ModalBody>
          {printingImg.state ? (
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="text-[18px] text-center">RESGATAR FOTO</p>
              {/* Campo de Inserção de Número (só aparece quando "IMPRIMINDO FOTO" está visível) */}
              <div className="mt-4 flex flex-col items-center gap-2">
                <div className="flex items-center gap-2">
                  <Button
                    style={{
                      backgroundColor: '#ab8654', // Cor de fundo (verde, exemplo)
                      ...
                      boxShadow: '0px 4px 6px rgba(0, 0, 0, 0.1)', // Sombra para o botão
                    }}
                    onPress={() => setNumberField((prev) => Math.max(0, prev - 1))}
                  >
                    -
                  </Button>
                  <input
                    ...
                    value={numberField}
                    onChange={(e) =>
                      setNumberField(Math.max(0, parseInt(e.target.value) || 0))
                    }
                ...
                <Button
                  style={{
                    backgroundColor: '#ab8654', // Cor de fundo específica (exemplo: laranja)
                    color: '#FFFFFF', // Cor do texto (branco)
                    border: 'none', // Sem borda
                    padding: '10px 20px', // Espaçamento interno
                    fontSize: '16px', // Tamanho da fonte
                  ...
                  onPress={() => printNumberToConsole(numberField)}
                >
                  Imprimir
                </Button>
      ...
      <Modal isOpen={isGalleryOpen} onOpenChange={onGalleryOpenChange} isDismissable={false} size="lg" scrollBehavior="inside" >
        <ModalContent>
          {(onGalleryClose) => (
            <>
              ...
              <ModalBody>
                {countdown !== null ? (
                  <div className="flex flex-col items-center">
                    <h2 className="text-2xl font-bold">Preparando...</h2>
                    <p className="text-lg">Foto será tirada em {countdown} segundos</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center">
                    <video
                      ...
                      playsInline
                      muted
                      className="border rounded-lg w-full h-[1700px] object-cover"
                    ></video>
                  </div>
                )}
              </ModalBody>
